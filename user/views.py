...
class RegisterProfessional(APIView):
    def post(self, request):
        serializer = ProfessionalRegisterSerializer(data=request.data)
        if serializer.is_valid():
            professional = serializer.save()
            return Response({'id': professional.id}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

class ProviderProfessional(APIView):
    def post(self, request):
        serializer = ProviderRegisterSerializer(data=request.data)
        if serializer.is_valid():
            provider = serializer.save()
            return Response({'id': provider.id}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


class ProviderUserRetrieveAPIView(RetrieveAPIView):
    queryset = Provider.objects.all()  # Assuming Provider is the model you are working with
    serializer_class = SimpleUserSerializer
    def get_object(self):
        admin_id = self.kwargs['admin_id']
        return self.queryset.get(admin_id=admin_id)
    
class ProfessionalUserRetrieveAPIView(RetrieveAPIView):
...
class UpdateProfessionalStatus(APIView):
